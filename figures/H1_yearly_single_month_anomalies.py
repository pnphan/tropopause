...
def H1_yearly_single_month_anomalies(month, station_list=station_list):
    ...
    #plt.show()
    plt.ylim((-1, 1))
    plt.title(f'Yearly {months_dict[month]} Anomaly of First Tropopause Height')
    plt.savefig(f'./figures/figures_output/H1_yearly_{month}_{months_dict[month]}_anomalies.png', dpi=1200, bbox_inches='tight')
    return x, y


if __name__ == "__main__":
    for month in months:
        H1_yearly_single_month_anomalies(month)