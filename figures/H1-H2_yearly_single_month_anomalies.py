...
def H2_H1_yearly_single_month_anomalies(month, station_list=station_list):
    monthly_means = {}
    current_month = []
    ...
    plt.title(f"Yearly {months_dict[month]} Anomaly of Tropopause Thickness")
    plt.xlabel('Year')
    plt.ylabel('Anomaly (km)')
    #plt.grid(True)
    #plt.show()
    plt.ylim((-1, 1))
    plt.savefig(f'./figures/figures_output/H1-H2_yearly_{month}_{months_dict[month]}_anomalies.png', dpi=1200, bbox_inches='tight')
    return x, y


if __name__ == "__main__":
    for month in months:
        H2_H1_yearly_single_month_anomalies(month)